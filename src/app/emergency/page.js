...
const Emergency = () => {
  ...
  return (
    <div className='container mx-auto mt-10'>
      <Link href='/'><h1 className='text-center text-2xl md:text-3xl text-red-500 font-bold mb-5 mt-10'>Emergency Notification Form</h1></Link>
      {notificationSent ? (
        Swal.fire(
          'Sent!',
        ...
        <form onSubmit={handleSubmit} className='mt-10 justify-center mx-auto bg-red-50 p-10 card flex-shrink-0 w-full shadow-2xl max-w-4xl rounded-lg py-8 px-10 flex flex-grow overflow-y-auto'>
          <div className='flex'>
            ...
            <textarea
              id="message"
              placeholder='Message'
              className='ms-2 p-2 border border-red-500'
              value={message}
              rows={4}
              cols={60}
  ...
  <label className='text-md md:text-lg font-bold label cursor-pointer'>
    Recipients:
  </label>
  <div className='flex flex-wrap'>
    {bloodTypes.map((bloodType) => (
      <label className='label cursor-pointer' key={bloodType}>
        ...
        {bloodType}
          ...
          <div className='flex mt-3'>
            ...
            <textarea className="message-preview border border-red-500 italic overflow-y-auto ms-2 p-2" placeholder='Preview Message' rows={4}
              cols={50}>{message}</textarea>
          </div>
          <button type="submit" className='mx-auto w-48 btn btn-outline btn-error mt-5'>Send Notification</button>
        </form>

      )}
      <div>
        <h2 className='text-center text-2xl md:text-3xl text-red-500 font-bold mb-5 mt-10'>Emergency Tips</h2>
